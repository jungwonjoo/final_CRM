...
import { supports, recentRegistered, recentSearched } from "@/lib/mock-data";

export const metadata = { title: "고객지원 · FingerSales" };
  ...
  return (
    <div className="flex flex-col gap-4">
      <nav className="flex items-center justify-end gap-1 text-xs text-muted-foreground">
        ...
        <span>영업관리</span>
        <ChevronRight className="size-3.5" />
        <span className="text-foreground">고객지원</span>
      </nav>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[260px_minmax(0,1fr)_260px]">
        ...
        <section className="flex flex-col">
          <div className="mb-3 flex items-center gap-2">
            <h1 className="text-lg font-bold">고객지원</h1>
            <span className="text-sm text-muted-foreground">
              ( 검색결과 : {supports.length} 건 )
            </span>
            <Link
              href="/support/new"
              title="고객지원 추가"

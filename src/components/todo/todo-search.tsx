...
const kinds = ["전화", "메일", "방문", "견적", "기타"];
const statuses = ["진행중", "완료"];
const teams = ["영업팀", "개발팀", "기술연구소", "클라우드팀", "마케팅팀", "경영지원팀"];
const sorts = ["등록일", "고객사", "기한일"];
      ...
      <div className={cn("mt-3 flex-col gap-2.5 lg:flex", open ? "flex" : "hidden")}>
        <Select>
          <SelectTrigger className="w-full">
            <SelectValue placeholder="할일구분(전체)" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">할일구분(전체)</SelectItem>
            {kinds.map((k) => (
        ...
        <Select>
          <SelectTrigger className="w-full">
            <SelectValue placeholder="할일상태(전체)" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">할일상태(전체)</SelectItem>
            {statuses.map((s) => (

...
import { ProceedingSearch } from "@/components/proceedings/proceeding-search";
import { ProceedingCard } from "@/components/proceedings/proceeding-card";
import { RecentSection } from "@/components/layout/recent-section";
import { EmptyState } from "@/components/common/empty-state";
import { proceedings, recentRegistered, recentSearched } from "@/lib/mock-data";

export const metadata = { title: "회의록 · FingerSales" };

export default function ProceedingListPage() {
  return (
    <div className="flex flex-col gap-4">
      <nav className="flex items-center justify-end gap-1 text-xs text-muted-foreground">
        ...
        <span>영업도구</span>
        <ChevronRight className="size-3.5" />
        <span className="text-foreground">회의록</span>
      </nav>
